...
1\. Create or update \`project\_specs.md\` and define:  
...
\- **\*\*Language:\*\*** TypeScript  
\- **\*\*Framework:\*\*** [Next.js](http://Next.js)@latest (App Router. Website must be built in [Next.js](http://Next.js) \- do not build a static HTML site unless explicitly asked.  
...
\- \`project\_specs.md\` → The blueprint Claude reads before doing anything
...
\- Never expose the \`service\_role\` key in client-side code
...
\- Never expose Supabase \`service\_role\` key in frontend code  
...
Only build what is described in \`project\_specs.md\`.  
